...
public:
    Game(string proc_name) : name(proc_name), pid(stoi(run("pidof " + proc_name))) {
        // See: proc(5)
    ...
    void search_for(uint32_t val) {
        if (remain.size() == 0) {
            ...
            std::regex r(R"(^([0-9a-f]+)\s+(\d+)\s+(\d+)\s+(\d+)\s+rw.*)");
            ...
            std::erase_if(remain, [this, val](uintptr_t addr) { return load(addr) != val; });
        }
        printf("There are %ld match(es).\n", remain.size());
    }

    void reset() { remain.clear(); }
